...
class miniorange_configure_method extends FormBase
{
    ...
    public function moGenerateInlineForm(array &$form, $challengeResponse, $authMethod)
    {
      ...
      if (in_array($authMethod, $utilities->mo_TOTP_2fa_mentods())) {
            $form['mo_auth_totp_label'] = array(
                ...
                '#title' => '<b>' . t('Step 1: ') . '</b>' . t('@challengeResponse_message', ['@challengeResponse_message'=>$challengeResponse->message]),
            );
            ...
            $form['mo_auth_actions_qrcodeData'] = array(
              '#markup' => $qrCode,
              '#prefix' => '<div class="form-item--editor-format">',
              '#suffix' => '	&nbsp;&nbsp;',
            );
            $secret = $utilities->indentSecret($challengeResponse->secret);
            $form['mo_auth_actions_secret_key'] = array(
              '#markup' => '<div class="qr_code_text">
                                <p><b>'.t('Can\'t scan the code? ').'</b>' .t('Add the code to your app manually:').'</p>
                                <p id="moTwoFAAuthSecret"><b>' . t('Key: ') . '</b><code style="background-color: #e0e0e0; border-radius: 4px; padding: 0.55em;"><b>' . $secret . '</b></code> (' . t('Spaces does\'t matter') . ')</p>
                                <p><b>'.t('Time based: ').'</b>'.t('Yes').'</p>
                              </div></div>'
            );
            $form['mo_auth_passcode_textfield'] = [
                '#type' => 'textfield',
                '#title' => '<b>' . t('Step 2:') . '</b>' . t('Enter the passcode generated on your @app_name app', ['@app_name'=>$app_name]),
    ...
    {
      $utilities = new MoAuthUtilities();
      if($form_state->getValue('mo_auth_answer1') !== null) {
