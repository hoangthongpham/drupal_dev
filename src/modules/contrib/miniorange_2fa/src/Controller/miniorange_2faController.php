...
class miniorange_2faController extends ControllerBase
{
    ...
    public function openCustomKBAQuesForm()
    {
        $response = new AjaxResponse();
        $modal_form = $this->formBuilder->getForm('\Drupal\miniorange_2fa\Form\MoCustomKBAQues');
        $response->addCommand(new OpenModalDialogCommand('Customize KBA Questions', $modal_form, ['width' => '60%']));
        return $response;
    }
    ...
    public function openConfigureAllowedMethods()
    {
        $response = new AjaxResponse();
        $modal_form = $this->formBuilder->getForm('\Drupal\miniorange_2fa\Form\MoAuthAllowedMethods');
        $response->addCommand(new OpenModalDialogCommand('Set Up Available 2FA Methods ', $modal_form, ['width' => '60%']));
        return $response;
    }
     ...
     * @return RedirectResponse
     */
    function moUpdateTwoFactorStatus(AccountInterface $user, $enabled)
    {
      $uid = $user->id();
      MoAuthUtilities::update_user_status_from_UserAuthentication_table($uid,$enabled);
