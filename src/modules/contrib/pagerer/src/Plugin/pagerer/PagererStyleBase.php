...
use Drupal\pagerer\PagererManager;
...
abstract class PagererStyleBase extends PluginBase implements PagererStyleInterface, PluginFormInterface, ContainerFactoryPluginInterface {
  ...
  public function preprocess(array &$variables): void {
    ...
    if ($this->pager->getTotalPages() < (int) $this->getOption('display_restriction')) {
      return;
    }
   ...
   *   Render array.
   */
  abstract protected function buildPagerItems() :array;
   ...
   */
  protected function getOption(string $key): int|float|string|array|NULL {
    $keys = explode('.', $key);
  ...
  protected function getPageItem(int $offset, string $offset_mode = 'absolute', bool $progr_page = FALSE, string $title_tag = 'page', bool $set_query = TRUE): array {
    ...
    $ret = [
      ...
      'href' => $this->getPagererManager()->getHref($this->pager, $this->parameters, $this->pager->getCurrentPage() + $offset, [], $set_query),
  ...
  protected function getNavigationItem(string $scope, bool $href = TRUE): array {
    ...
    // active or not.
    $offset = match ($scope) {
      'first'    => -$this->pager->getCurrentPage(),
      'previous' => ($this->pager->getCurrentPage() == 0) ? 0 : -1,
      'next'     => ($this->pager->getCurrentPage() == $this->pager->getLastPage()) ? 0 : 1,
      'last'     => $this->pager->getLastPage() - $this->pager->getCurrentPage(),
      default    => 0,
    };
    ...
    if ($href) {
      $ret['href'] = $this->getPagererManager()->getHref($this->pager, $this->parameters, $this->pager->getCurrentPage() + $offset);
    }
  ...
  protected function prepareJsState(array &$state_settings): string {
    ...
    // 'value' attribute.
    $query = $this->getPagererManager()->getPagererUpdatedParameters($this->pager, $this->parameters, 'pagererpage');
    ...
    $default_settings = [
      'url'            => $this->getPagererManager()->getHref($this->pager, [], 0, [], FALSE)->toString(),
  ...
  /**
   * Returns the PagererManager.
   *
   * @return \Drupal\pagerer\PagererManager
   *   The pagerer manager that extends PagerManager.
   */
  protected function getPagererManager(): PagererManager {
    assert($this->pagerManager instanceof PagererManager);
