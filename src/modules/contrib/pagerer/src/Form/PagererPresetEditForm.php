...
class PagererPresetEditForm extends PagererPresetFormBase {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValues()) {
      ...
      foreach (['left', 'center', 'right'] as $pane) {
        $this->getPagererPresetEntity()->setPaneData($pane, 'style', $form_state->getUserInput()['panes_container'][$pane]['style']);
      }
    }

    $form['#title'] = $this->t("Edit pager %preset_label", ['%preset_label' => $this->getPagererPresetEntity()->label()]);
    ...
    foreach (['left', 'center', 'right'] as $pane) {
      ...
      $style = $this->getPagererPresetEntity()->getPaneData($pane, 'style');
    ...
    $form['preview']['pagerer'] = [
      ...
      '#config' => [
        'panes' => [
          'left' => $this->getPagererPresetEntity()->getPaneData('left'),
          'center' => $this->getPagererPresetEntity()->getPaneData('center'),
          'right' => $this->getPagererPresetEntity()->getPaneData('right'),
        ],
      ],
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    // Set data.
    foreach (['left', 'center', 'right'] as $pane) {
      $this->getPagererPresetEntity()->setPaneData($pane, 'style', $form_state->getValue([
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if (strpos($trigger, 'config', 0) === 0) {
      ...
      $form_state->setRedirect(
        ...
        ['pagerer_preset' => $this->getPagererPresetEntity()->id(), 'pane' => $pane]
      );
    }
    elseif (strpos($trigger, 'reset', 0) === 0) {
      ...
      $this->messenger->addMessage($this->t('Changes to the pager %label have been saved.', ['%label' => $this->getPagererPresetEntity()->label()]));
    }
