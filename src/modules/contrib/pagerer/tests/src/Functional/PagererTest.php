  ...
  public function testQuerystringOverrides(): void {
    ...
    foreach (['page', 'pg_0', 'px_1'] as $set) {
      ...
      foreach ($test_data as $data) {
        ...
        $multiple_pagers_path = Url::fromUri('base:pager-test/multiple-pagers', ['external' => TRUE, 'absolute' => TRUE])->toString();
        $this->drupalGet($multiple_pagers_path . $input_query, ['external' => TRUE]);
        $this->assertSession()->statusCodeEquals(200);
        foreach (array_keys($expected_markup) as $pager_element) {
          $active_pages = $this->cssSelect("div.test-pager-{$pager_element} ul.pager__items li.is-active");
          $this->assertNotEmpty($active_pages, $data['input_querystring'] . " // $pager_element // $set");
          $active_page = $active_pages[0];
          if (version_compare(\Drupal::VERSION, '10.3', '<')) {
            $this->assertEquals("Current page {$expected_markup[$pager_element]}", $active_page->getText());
          }
          else {
            $this->assertEquals("Page {$expected_markup[$pager_element]}", $active_page->getText());
          }
          $destination = $active_page->find('css', 'a')->getAttribute('href');
